...
def get_detections3(encoder, bgr_image, frame_idx, threshold, video_name):
    # rows are the detections resulting from loading SSD in the MOT format
    rows = model.run_inference_for_single_image(bgr_image, frame_idx, threshold, video_name)
    if len(rows) <= 0:
        return None
    features = encoder(bgr_image, rows[:, 2:6].copy())
    return [np.r_[(row, feature)] for row, feature
                       in zip(rows, features)]
# def get_detections3(encoder, bgr_image, frame_idx, threshold):
#     # rows are the detections resulting from loading SSD in the MOT format
#     rows, mask = model.run_inference_for_single_image(bgr_image, frame_idx, threshold)
#     if len(rows) <= 0:
#         return None
...
#     if mask:
#         return [np.r_[(row, feature)] for row, feature
#                        in zip(rows, features)], mask
    
#     return [np.r_[(row, feature)] for row, feature
#                        in zip(rows, features)], None
