...
class Model(object):
	...
	def convert_output(self, output_dict, iframe, width, height, threshold):
		...
		for i in range(len(output_dict['detection_boxes'])):
			if output_dict['detection_scores'][i] >= threshold: # and output_dict['detection_classes'][i] == 9:
	...
	def run_inference_for_single_image(self, image, image_index, threshold, video_name):
		with self.detection_graph.as_default():
			...
			if 'detection_masks' in tensor_dict:
				...
				detection_masks_reframed = utils_ops.reframe_box_masks_to_image_masks(detection_masks, detection_boxes, image_np_expanded.shape[1], image_np_expanded.shape[2])
			...
			if 'detection_masks' in output_dict:
				...
				# print(image.shape)
				# # cv2.imwrite(os.path.join('../{}/{}'.format(video_name, 'masks')), '{}.jpg'.format(image_index), image)
				# cv2.imwrite(os.path.join('{}/{}'.format(video_name, 'masks'), 
    #             '{}.jpg'.format(str(image_index).zfill(10))), 
    #             255 * output_dict['detection_masks'][0, :, :])
